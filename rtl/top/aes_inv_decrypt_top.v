module aes_decrypt_top(
    ...
    always @(*) begin
        case(round)
            ...
            4'd1: current_round_key = k9;
            4'd2: current_round_key = k8;
            4'd3: current_round_key = k7;
            4'd4: current_round_key = k6;
            4'd5: current_round_key = k5;
            4'd6: current_round_key = k4;
            4'd7: current_round_key = k3;
            4'd8: current_round_key = k2;
            4'd9: current_round_key = k1;
    ...
    wire [127:0] final_sr;
    wire [127:0] final_sb;
    wire [127:0] final_plain;

    inv_shiftrows FINAL_SR(
        .state_in(state_reg),
        .state_out(final_sr)
    );

    inv_subbytes FINAL_SB(
        .state_in(final_sr),
        .state_out(final_sb)
    );
    ...
    always @(posedge clk) begin
        ...
        if(rst) begin
            ...
            case(state)
                ...
                // Initial AddRoundKey(K10)
                LOAD
                :
                begin
                    state_reg <= ciphertext ^ k10;
                    round     <= 4'd1;
                    state     <= ROUND;
                end
